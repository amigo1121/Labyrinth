...
document.addEventListener(
   ...
   function (event) {
      ...
      if (
         ...
         if (players[currentPlayerNumber].id != parseInt(dragged.id)) {
            alert('Not your turn!');
            return;
         }
...
function updatePlayerInfo() {
   ...
   if (players.length > 0) {
      cur.innerHTML = `Current player: ${players[currentPlayerNumber].color}`;
      info.appendChild(cur);
      cur = document.createElement('ul');
      players.forEach(p => {
         cur.innerHTML += singlePlayerInfo(p);
      })
      info.appendChild(cur);
   }

}
function singlePlayerInfo(p) {
   return (`
   <li>Color: ${p.color}
   <ul>
      <li>Need to collect: ${p.items}</li>
  </ul>
   </li>
  `)
}